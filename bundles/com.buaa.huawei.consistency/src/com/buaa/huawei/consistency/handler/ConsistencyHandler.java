...
public class ConsistencyHandler extends AbstractHandler {
	...
	public Object execute(ExecutionEvent event) throws ExecutionException {
		...
		if (projects.length <= 0) {
			...
			if (mode == null) {
				...
				switch (mode) {
				case Mode.mode1:
					// consistency_check_result = checkModelAndProject(window.getShell(), model,
					// projects);
					break;
				case Mode.mode2:
	 ...
	 */
	/*
	 * private HashMap<String, HashMap<String, Pair<?, ?>>> checkModelAndProject(Shell shell, Model
	 * model, IProject[] projects) { // TODO Auto-generated method stub // 用户在弹出的SelectionDialog中进行操作
	 * Mode1SelectionDialog selection_dialog = new Mode1SelectionDialog(shell, model.getName(),
	 * projects); Pair<String, Integer> index_pair = selection_dialog.getSelection(); if (index_pair !=
	 * null) { // 获取模型的各种关系 Pair<String, HashMap<String, HashMap<String, ?>>> base_hierarchy =
	 * ModelHierarchyGetter.getHierarchy(model, index_pair.getLeft()); // 获取待查版本的调用、继承、实现关系 Pair<String,
	 * HashMap<String, HashMap<String, ?>>> unchecked_hierarchy =
	 * ProjectHierarchyGetter.getHierarchy(projects[index_pair.getRight().intValue() ]);
	 * 
	 * // 下面的还得改！！！！！！！！！！！！！！！！！！！！ ModelAndProjectConsistencyChecker mp_checker = new
	 * ModelAndProjectConsistencyChecker(base_hierarchy, unchecked_hierarchy, index_pair.getLeft());
	 * return mp_checker.checkConsistency(); } else { System.err.println("由于没有选择模型图类型或项目版本，所以接下来什么也不做");
	 * return null; } }
	 */
	 ...
	 */
	/*
	 * private Model getModel() { // TODO Auto-generated method stub Model model = null; IWorkbenchPage
	 * workbench_page = PlatformUI.getWorkbench().getActiveWorkbenchWindow().getActivePage();
	 * IViewReference[] view_refs = workbench_page.getViewReferences(); for (IViewReference view_ref :
	 * view_refs) { IViewPart view_part = view_ref.getView(false); // 获取Model Explorer视图 if (view_part
	 * instanceof ModelExplorerPageBookView) { workbench_page.activate(view_part); ModelExplorerView
	 ...
	 * models[0].getData()).getEObject()); } } } } return model; }
	 */
